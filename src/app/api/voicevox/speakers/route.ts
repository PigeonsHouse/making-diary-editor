...
export async function GET() {
  try {
    ...
    return NextResponse.json(speakers.map((speaker: {name: string; styles: Array<{name: string; id: number}>}) => ({
      name: speaker.name,
      styles: speaker.styles.map((style) => ({name: style.name, id: style.id})),
    })));
  } catch (error) {
    return NextResponse.json({error: error instanceof Error ? error.message : "VOICEVOXへ接続できません"}, {status: 503});
  }
}
