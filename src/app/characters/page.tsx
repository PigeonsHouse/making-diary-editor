        ...
        {row ? (
          <section className="character-form">
            ...
            <div className="field-grid">
              ...
              <label>VOICEVOX名<input value={row.data.voicevoxName} onChange={(event) => update((draft) => { draft.voicevoxName = event.target.value; })} /></label>
              <label>イメージカラー<input type="color" value={row.data.color} onChange={(event) => update((draft) => { draft.color = event.target.value; })} /></label>
              <label>既定スタイルID<input type="number" value={row.data.voice.styleId} onChange={(event) => update((draft) => { draft.voice.styleId = Number(event.target.value); })} /></label>
            ...
            <div className="psd-placeholder"><strong>PSD立ち絵</strong><p>PSDアップロードとレイヤー選択は素材API接続後に利用できます。</p></div>
          </section>
