...
new Worker("renders", async (queueJob) => {
  ...
  const inputProps = {project: job.snapshot, characters: characterRows.map((row) => row.data)};
  const outputPath = path.join(rendersDir, `${renderJobId}.mp4`);
  try {
    await db.update(renderJobs).set({status: "rendering", updatedAt: new Date()})
    ...
    await renderMedia({
      ...
      onProgress: async ({progress}) => {
        ...
        await db.update(renderJobs).set({progress: percent, updatedAt: new Date()})
          .where(eq(renderJobs.id, renderJobId));
      },
    });
...
new Worker("assets", async (queueJob) => {
  ...
  try {
    ...
    const args = asset.kind === "video"
      ? ["-y", "-i", asset.originalPath, "-map_metadata", "0", "-c:v", "libx264", "-pix_fmt", "yuv420p", "-c:a", "aac", "-movflags", "+faststart", output]
      : ["-y", "-i", asset.originalPath, "-map_metadata", "0", "-q:v", "2", output];
    await execFileAsync("ffmpeg", args);
    const {stdout} = await execFileAsync("ffprobe", [
      "-v", "error", "-show_entries", "stream=width,height,duration:format=duration",
      "-of", "json", output,
    ]);
    await db.update(assets).set({
