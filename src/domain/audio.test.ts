...
import { getBgmVolume, groupContinuousBgm, resolveSoundEffect } from "./audio";
...
describe("resolveSoundEffect", () => {
  it("inherits the project default", () => {
    expect(resolveSoundEffect(projectDefault, { mode: "inherit" })).toBe(projectDefault);
  });
  ...
  it("uses a custom sound instead of the project default", () => {
    const custom = { ...projectDefault, volume: 0.4 };
    expect(resolveSoundEffect(projectDefault, { mode: "custom", clip: custom })).toBe(custom);
  });
});
...
describe("groupContinuousBgm", () => {
  it("keeps the same BGM running across adjacent scenes", () => {
    const segments = groupContinuousBgm([
      { key: "wish", from: 0, duration: 90, bgm: projectDefault },
      { key: "diary-1", from: 90, duration: 120, bgm: { ...projectDefault, volume: 0.5 } },
    ]);
  ...
  it("starts a new loop after BGM is disabled or changed", () => {
    ...
    const segments = groupContinuousBgm([
      { key: "a", from: 0, duration: 30, bgm: projectDefault },
      { key: "b", from: 30, duration: 30, bgm: null },
      { key: "c", from: 60, duration: 30, bgm: projectDefault },
      { key: "d", from: 90, duration: 30, bgm: other },
    ]);
