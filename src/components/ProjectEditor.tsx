...
import type {Character, ContentBlock, Dialogue, ProjectDocument, ProjectRecord} from "@/domain/types";
          ...
          <div className="editor-tabs-toolbar">
            ...
            <div className="editor-tab-actions">
              {project.diaries.length > 1 ? (
                <button className="secondary sort-diaries" onClick={sortDiaries}>
                  日付順
                </button>
              ) : null}
              <button className="primary add-diary-tab" onClick={addDiary}>
                ＋ 日誌を追加
              </button>
            </div>
          </div>
              ...
              >
                ...
                <article className="diary-card selected">
                  <div className="diary-heading">
                    ...
                    />
                    <button className="icon danger" title="この日誌を削除" onClick={() => removeDiary(diaryIndex)}>
                      ×
                    </button>
                  </div>
      ...
      <div className="project-character-layout">
        ...
        {project.characterIds.map((characterId) => {
          ...
          return (
            <label
              key={characterId}
              style={{"--character-color": character?.color ?? "#64748b"} as React.CSSProperties}
            >
              {character?.name ?? "不明"}
              <span>X</span>
              <input
...
function DialogueEditor({
  ...
  return (
    <div
      ...
      style={{"--speaker": character?.color ?? "#64748b"} as React.CSSProperties}
      onDragOver={(event) => {
...
function DialogueVoiceOverrides({
  character,
  dialogue,
  updateDialogue,
}: {
...
function AssetLibrary({assets, onChanged}: {assets: AssetRow[]; onChanged: (assets: AssetRow[]) => void}) {
