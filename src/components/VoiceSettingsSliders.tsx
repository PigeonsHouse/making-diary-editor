...
export function VoiceSettingsSliders({values, defaults, onChange, allowUnset = false}: {
  values: Partial<AdjustableValues>;
  defaults: AdjustableValues;
  onChange: (key: AdjustableKey, value: number | undefined) => void;
  allowUnset?: boolean;
}) {
  return <div className="voice-sliders">{(Object.keys(configs) as AdjustableKey[]).map((key) => {
    const overridden = values[key] !== undefined;
    return <AdaptiveSlider key={key} config={configs[key]}
      value={values[key] ?? defaults[key]} disabled={allowUnset && !overridden}
      multiplierEnabled={overridden || !allowUnset}
      onChange={(value) => onChange(key, value)}
      action={allowUnset ? <button type="button" className="voice-override-toggle" onClick={() =>
        onChange(key, overridden ? undefined : defaults[key])}>
        {overridden ? "既定に戻す" : "上書き"}
      </button> : null} />;
  })}</div>;
}

function AdaptiveSlider({config, value, disabled, multiplierEnabled, onChange, action}: {
  config: {label: string; min: number; baseMax: number; step: number; symmetric?: boolean};
  value: number; disabled: boolean; multiplierEnabled: boolean;
  onChange: (value: number) => void; action: React.ReactNode;
}) {
  ...
  return <div className={`voice-slider ${disabled ? "disabled" : ""}`}>
    ...
    <input type="range" min={config.symmetric ? -maximum : config.min} max={maximum} step={config.step} value={value}
      disabled={disabled}
      onChange={(event) => {
      ...
      onPointerDown={() => { interactionValue.current = value; }}
